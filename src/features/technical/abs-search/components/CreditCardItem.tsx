        ...
        loanSoapService.getLoanDetails(credit.referenceId!)
          .then((details: any) => {
            ...
            if (details && details.balances) {
              const sum = details.balances.reduce((acc: number, b: any) => acc + Number(b.balance || 0), 0);
        ...
        absService.getCreditGraphs(credit.referenceId!)
          .then((graphs: any[]) => {
            ...
            if (graphs && graphs.length > 0) {
              ...
              let targetDate = sortedDates.find(d => new Date(d).getTime() >= now);
              ...
              if (targetDate) {
                ...
                let sum = 0;
                payments.forEach((p) => {
                  if (p.LongName === 'Основной долг' || p.LongName === 'Проценты по кредиту') {
    ...
    <Card className="rounded-3xl p-6 bg-white border-none shadow-sm flex flex-col gap-6 relative overflow-hidden">
      ...
      <div className="flex justify-between items-start">
        <div className="space-y-2">
          <div className="flex items-center gap-3">
            <h3 className="text-xl font-bold text-slate-900">{credit.productName?.replace(/\\"/g, '"') || '-'}</h3>
            {credit.statusName && (
          ...
          <div className="text-sm text-slate-400">
            Дата открытия: {formatDate(credit.documentDate)} | Отдел: {departmentText}
          </div>
        </div>
        <div className="text-right">
          <div className="text-xs text-slate-400 mb-1">Остаток задолженности</div>
      ...
      <div className="flex justify-between items-end pt-2">
        ...
        <Button 
          ...
          onClick={() => credit.referenceId && onOpenDetails(credit.referenceId)}
        >
          Подробнее
        </Button>
      </div>
