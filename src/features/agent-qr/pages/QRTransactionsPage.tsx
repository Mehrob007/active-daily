...
export default function QRTransactionsPage() {
  ...
  const [type, setType] = useState<'usOnThem' | 'themOnUs'>('themOnUs');
  ...
  const fetchData = useCallback(async () => {
    setIsLoading(true);
    try {
      const data = await qrAgentService.getTransactions(type, startDate, endDate, 1, PAGE_SIZE);
  ...
  const handleExport = async () => {
    setIsExporting(true);
    try {
      const blob = await qrAgentService.exportTransactions(type, filteredData);
